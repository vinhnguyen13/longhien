...
class Image_GD_Driver extends Image_Driver {
	...
	public function watermark($Watermarkpng) {
		$params = CParams::load ();
		if (file_exists($Watermarkpng)) {
			$newimage=imagecreatefrompng($Watermarkpng);
			$wpos=explode(' ',str_replace('%','', $params->params->uploads->photo->thumbwatermark->watermarkposition));
			$check	=	imagecopymerge($this->tmp_image, $newimage,min(max(imagesx($this->tmp_image)*($wpos[0]/100)-0.5*imagesx($newimage),0),imagesx($this->tmp_image)-imagesx($newimage)),min(max(imagesy($this->tmp_image)*($wpos[1]/100)-0.5*imagesy($newimage),0),imagesy($this->tmp_image)-imagesy($this->tmp_image)),0,0,imagesx($newimage),imagesy($newimage),intval($params->params->uploads->photo->thumbwatermark->watermarktransparency));
	...
	public function logo($logo) {
		...
		if (file_exists($logo)) {
			/*
			$newimage=imagecreatefrompng($logo);
			$wpos=explode(' ',str_replace('%','', $params->params->uploads->photo->thumbwatermark->watermarkposition));
			$check	=	imagecopymerge($this->tmp_image, $newimage,min(max(imagesx($this->tmp_image)*($wpos[0]/100)-0.5*imagesx($newimage),0),imagesx($this->tmp_image)-imagesx($newimage)),min(max(imagesy($this->tmp_image)*($wpos[1]/100)-0.5*imagesy($newimage),0),imagesy($this->tmp_image)-imagesy($this->tmp_image)),0,0,imagesx($newimage),imagesy($newimage),intval(0));
			imagedestroy($newimage);
			return $check;
			*/
			...
			$check	=	imagecopymerge($this->tmp_image, $stamp, imagesx($this->tmp_image) - $sx - $marge_right, imagesy($this->tmp_image) - $sy - $marge_bottom, 0, 0, imagesx($stamp), imagesy($stamp), 20);
